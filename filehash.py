...
There's no warm up, there's no cache/memory/whatever filling. It's just a simple way to explore performance for
...
def get_timings():
    funcList = [f for f, v in inspect.getmembers(sys.modules['__main__'], callable) if f.startswith('t_')]
...
def t_md5sum(filename="big.txt", block_size=2 ** 16):
    md5 = hashlib.md5()
...
def t_sha1sum(filename="big.txt", block_size=2 ** 16):
    sha1 = hashlib.sha1()
    with open(filename, 'rb') as f:
        for chunk in iter(lambda: f.read(block_size), b''):
            sha1.update(chunk)
...
def t_sha224sum(filename="big.txt", block_size=2 ** 16):
    sha = hashlib.sha224()
    with open(filename, 'rb') as f:
        for chunk in iter(lambda: f.read(block_size), b''):
            sha.update(chunk)
...
def t_sha256sum(filename="big.txt", block_size=2 ** 16):
    sha = hashlib.sha256()
    with open(filename, 'rb') as f:
        for chunk in iter(lambda: f.read(block_size), b''):
            sha.update(chunk)
...
def t_sha384sum(filename="big.txt", block_size=2 ** 16):
    sha = hashlib.sha384()
    with open(filename, 'rb') as f:
        for chunk in iter(lambda: f.read(block_size), b''):
            sha.update(chunk)
...
def t_sha512sum(filename="big.txt", block_size=2 ** 16):
    sha = hashlib.sha512()
    with open(filename, 'rb') as f:
        for chunk in iter(lambda: f.read(block_size), b''):
            sha.update(chunk)
...
def tno_crc32sum(filename="big.txt", block_size=2 ** 16):
    checksum = 0
    with open(filename, 'rb') as f:
        for chunk in iter(lambda: f.read(block_size), b''):
